...
export default function Header() {
  ...
  useEffect(() => {
    const handleSettingsChange = () => {
      ...
      console.log('Header settings updated:', newSettings);
    };
            ...
            <nav className="hidden md:flex items-center gap-1">
              {mainNav.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="px-3 py-2 text-sm font-medium text-gray-700 hover:text-primary-600 transition-colors cursor-pointer"
                >
                  {item.name}
                </Link>
              ))}
          ...
          <div className="flex items-center gap-4">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2">
                <Link 
                  href="/admin"
                  className="text-primary-600 font-medium text-sm hover:text-primary-700 px-3 py-2 rounded-lg hover:bg-primary-50 transition-colors"
                >
                  后台管理
                </Link>
